...
# Opposite of get_fips
def get_state(fips):
    with open('fips_dict.json', 'r') as f:
        fips_dict = json.load(f)

    for state, details in fips_dict.items():
        if details['state_fips'] == fips:
            return state
    return None
...
def fetch_OD(muni, state, year, direction):
    ...
    if year not in valid_years:
        print("Year not found. Please make sure to use YYYY, and note that data is available only from 2003 to 2022.")
        return None
    ...
    if dfs:
        ...
        print("Number of workers from municipality:", sum(From_muni['tot_jobs']))
        ...
        print("Number of states where workers in municipality are commuting to:", len(From_states))
        # group data by destination state
        From_muni_per_state = From_muni_sum.groupby('fips')['tot_jobs'].sum().reset_index()
        for index, row in From_muni_per_state.iterrows():
            From_muni_per_state.loc[index, 'state_name'] = get_state(row['fips']) 
        From_muni_per_state = From_muni_per_state.sort_values(by = 'tot_jobs', ascending=False).reset_index()
        From_muni_per_state = From_muni_per_state[['state_name', 'fips', 'tot_jobs']]
        print("States with highest number of workers from municipality commuting to state:")
        print(From_muni_per_state.head())
        ...
        print("Number of workers w jobs in municipality:", sum(To_muni['tot_jobs']))
        ...
        print("Number of states where workers from municipality are commuting from:", len(To_states))
         # group data by origin state
        To_muni_per_state = To_muni_sum.groupby('fips')['tot_jobs'].sum().reset_index()
        for index, row in To_muni_per_state.iterrows():
            To_muni_per_state.loc[index, 'state_name'] = get_state(row['fips']) 
        To_muni_per_state = To_muni_per_state.sort_values(by = 'tot_jobs', ascending=False).reset_index()
        To_muni_per_state = To_muni_per_state[['state_name', 'fips', 'tot_jobs']]
        print("States with highest number of workers commuting to municipality from state:")
        print(To_muni_per_state.head())
